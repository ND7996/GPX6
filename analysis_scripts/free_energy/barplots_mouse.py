...
# Load data
# -------------------------------
df = pd.read_csv(r'D:\PhD_Thesis\analysis\dipole\mouse_HBONDS.csv')

levels = sorted(df['level'].unique(), key=lambda x: int(x.strip().lower().replace('level', '')))
...
# -------------------------------
# Figure layout — dynamic to fit all levels
# -------------------------------
nrows = int(np.ceil(len(levels) / 4))
ncols = 4
n_slots = nrows * ncols

levels_padded = levels + [None] * (n_slots - len(levels))

fig, axes = plt.subplots(
    nrows,
    ncols,
    figsize=(20, nrows * 3.6),
    dpi=300
)
...
# Plot levels
# -------------------------------
for idx, level in enumerate(levels_padded):
    ...
    wt_row = level_data[level_data['mutation'].str.lower() == 'mousecys']
...
# -------------------------------
legend_elements = [
    plt.Rectangle((0, 0), 1, 1, fc='red',   alpha=0.9, label='WT (mousecys)'),
...
# Save
# -------------------------------
save_path = r'D:\PhD_Thesis\analysis\free_energy\mutation_mouse_all_levels.png'
