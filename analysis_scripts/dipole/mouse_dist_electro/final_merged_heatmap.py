...
# 1. Mouse activation energy data
mouse_df = pd.read_csv('mouse_data.csv')
print(f"✓ Mouse ΔG* data: {len(mouse_df)} rows")
...
# 3. H-bond data
hbond_df = pd.read_csv('hbonds.csv')
...
def extract_residue_number(mutation):
    """Extract residue number from mutation name like 'F48Y' -> '48'"""
    import re
    match = re.search(r'(\d+)', mutation)
    return match.group(1) if match else None

# Add residue numbers to mouse data
mouse_df['residue'] = mouse_df['mutation'].apply(extract_residue_number)

# Calculate average ΔG* across all levels for each mutation
avg_dg = mouse_df[mouse_df['mutation'] != 'mousecys'].groupby('residue').agg({
    'dg_star': 'mean',
    'dg0': 'mean',
    'mutation': 'first'
}).reset_index()
...
dist_pivot.columns = ['Residue', 'ResName', 'Dist_49', 'Dist_83', 'Dist_196']
...
for _, row in hbond_df.iterrows():
    ...
    # Check if involves active site (49, 83, 196)
...
merged_df['Min_Dist'] = merged_df[['Dist_49', 'Dist_83', 'Dist_196']].min(axis=1)
...
# ==============================================================================
# Column 1: ΔG‡ (Mouse) - SAME COLOR SCALE AS YOUR HEATMAP
# ==============================================================================
...
im2 = axes[1].imshow(data_dg0, aspect='auto', cmap='RdBu', vmin=-63, vmax=-53)
axes[1].set_yticks([])
axes[1].set_xticks([0])
...
axes[2].set_title('Distance to\nCYS49', fontsize=13, fontweight='bold', pad=10)
...
# ==============================================================================
# Column 5: Distance to RES196
# ==============================================================================
data_196 = merged_df[['Dist_196']].values
im5 = axes[4].imshow(data_196, aspect='auto', cmap='viridis_r', vmin=0, vmax=25)
axes[4].set_yticks([])
axes[4].set_xticks([0])
axes[4].set_xticklabels(['Dist to\nRES196\n(Å)'], fontsize=11, fontweight='bold')
axes[4].set_title('Distance to\nRES196', fontsize=13, fontweight='bold', pad=10)

for i in range(len(data_196)):
    if not np.isnan(data_196[i,0]):
        color = 'white' if data_196[i,0] > 12 else 'black'
        axes[4].text(0, i, f'{data_196[i,0]:.1f}', 
                    ha='center', va='center', fontsize=9, fontweight='bold', color=color)
...
# TITLE
# ==============================================================================
plt.suptitle('Comprehensive Mouse Enzyme Analysis: Activation Barriers, Distances, and H-bonding\n' +
             '(Sorted by distance | Red = High ΔG‡ [BAD] | Blue = Low ΔG‡ [GOOD] | ΔG* averaged across all mutation levels)', 
             fontsize=15, fontweight='bold', y=0.98)

plt.tight_layout()
plt.savefig('final_merged_heatmap.png', dpi=300, bbox_inches='tight')
print("\n✓ Saved: final_merged_heatmap.png")
...
corr_dg_196 = merged_df['dg_star'].corr(merged_df['Dist_196'])
...
print(f"\nΔG‡ vs Distance to CYS49:      r = {corr_dg_49:.3f}")
print(f"ΔG‡ vs Distance to GLN83:      r = {corr_dg_83:.3f}")
print(f"ΔG‡ vs Distance to RES196:     r = {corr_dg_196:.3f}")
...
print(f"\n✓ Saved merged data to: merged_analysis.csv")
