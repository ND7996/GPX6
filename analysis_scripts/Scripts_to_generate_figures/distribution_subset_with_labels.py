...
# Apply JCIM style (Arial 8pt, inward ticks, etc.)
set_jcim_style()

# Force all text to be black (JCIM requirement)
plt.rcParams.update({
    "text.color": "black",
...
_LABEL_OFFSETS = [
    dict(dx=-0.045, dy=-0.045, ha="right",  va="top"),
    dict(dx= 0.045, dy=-0.045, ha="left",   va="top"),
...
def draw_triangle(ax):
    ...
    for (vx, vy), col, label, off in zip(VERTICES, REF_COLORS, REF_LABELS, _LABEL_OFFSETS):
        ...
        # Text size handled by JCIM style (default 8pt)
        ax.text(vx + off["dx"], vy + off["dy"], label,
                ha=off["ha"], va=off["va"], zorder=9)
...
def main():
    ...
    for idx, (metric_label, metric_fn) in enumerate(METRICS):
        ...
        draw_triangle(ax)
        scatter_groups(ax, rows, label_lookup)
        finalise_panel(ax, metric_label)
        
        # Legend matching MPNN style exactly (frameon=False, fontsize=8, no title_fontsize)
        ...
        fig.savefig(out_s, dpi=300, bbox_inches="tight")
        plt.close(fig)
        print(f"  Saved: {out_s}\n")
