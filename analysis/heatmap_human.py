...
# Read the CSV file
file_path = r'D:\PhD_Thesis\GPX6\analysis\human_mutants_FINAL_with_distances.csv'
...
# Get WT values from Level0 humansec (the reference)
wt_row = df[(df['level'] == 'Level0') & (df['mutation'] == 'humansec')].iloc[0]
wt_dg_star = wt_row['dg_star']
wt_dg0 = wt_row['dg0']

print(f"\nWT (humansec) dg_star: {wt_dg_star}")
print(f"WT (humansec) dg0: {wt_dg0}")

# Remove only humansec rows for the heatmap (keep humancys)
df_mutants = df[df['mutation'] != 'humansec'].copy()

print(f"\nWorking with {len(df_mutants)} mutant rows (excluding humansec, keeping humancys)")
...
axes[0].set_title(f'ΔG* Heatmap (WT humansec = {wt_dg_star:.2f})', 
...
axes[1].set_title(f'ΔG0 Heatmap (WT humansec = {wt_dg0:.2f})', 
                  fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('mutant_heatmap_human_by_distance.png', dpi=300, bbox_inches='tight')
print("\nHeatmap saved as 'mutant_heatmap_human_by_distance.png'")
plt.show()
...
# Additional comparison: humancys vs humansec
print("\n=== Humancys (Cysteine) vs Humansec (Selenocysteine) Comparison ===")
humancys_data = df[df['mutation'] == 'humancys'][['level', 'dg_star', 'dg0']]
if len(humancys_data) > 0:
    print(f"Humancys dg_star: {humancys_data.iloc[0]['dg_star']:.2f} (diff from WT: {humancys_data.iloc[0]['dg_star'] - wt_dg_star:.2f})")
    print(f"Humancys dg0: {humancys_data.iloc[0]['dg0']:.2f} (diff from WT: {humancys_data.iloc[0]['dg0'] - wt_dg0:.2f})")