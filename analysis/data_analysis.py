...
# =================== ΔΔG CALCULATION (exactly as before) ===================
...
# =================== 1. FINAL LFER – TINY ELEGANT POINTS ===================
...
sc = ax.scatter(dfp['ΔΔG⁰'], dfp['ΔΔG‡'],
                ...
                s=16, edgecolors='black', linewidth=0.4, alpha=0.94, zorder=3)  # ← tiny!
...
text = f'Φ = {slope:.2f} ± {std_err:.2f}\nR² = {r_value**2:.3f}'
ax.text(0.04, 0.96, text, transform=ax.transAxes, fontsize=17,
        va='top', bbox=dict(boxstyle='round,pad=0.7', facecolor='white', edgecolor='black', alpha=0.97))
...
print(f"LFER → Φ = {slope:.3f} ± {std_err:.3f} | R² = {r_value**2:.3f} | n = {mask.sum()}")
...
# =================== 3. DISTANCE VS EFFECT (tiny points) ===================
if dist_col:
    plt.figure(figsize=(9,6))
...
# =================== 4. CLEAN BRØNSTED (tiny points) ===================
plt.figure(figsize=(8.5,6.5))
plt.scatter(dfp['ΔΔG⁰'], dfp['ΔΔG‡'],
            c=dfp['level_num'], cmap='viridis_r',
            s=16, edgecolors='black', linewidth=0.4, alpha=0.94)
plt.plot(x_fit, y_fit, '#d62728', lw=3.5)
plt.xlabel(r'$\Delta\Delta$G° (kcal mol⁻¹)')
plt.ylabel(r'$\Delta\Delta$G‡ (kcal mol⁻¹)')
plt.title(f'Brønsted Plot – Φ = {slope:.2f}')
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(outdir / "4_Bronsted.png", dpi=600)
plt.show()

# =================== 5. Φ DISTRIBUTION ===================
dfp['phi_ind'] = dfp['ΔΔG‡'] / dfp['ΔΔG⁰'].replace(0, np.nan)
plt.figure(figsize=(8,5))
plt.hist(dfp['phi_ind'].dropna(), bins=35, color='lightcoral', edgecolor='black', alpha=0.85)
plt.axvline(slope, color='red', lw=3, ls='--', label=f'Global Φ = {slope:.2f}')
plt.xlabel('Individual Φ value')
plt.ylabel('Count')
plt.title('Distribution of Local Φ Values')
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(outdir / "5_Phi_distribution.png", dpi=600)
plt.show()

# =================== 6. HEATMAP – FULLY PRESERVED ===================
...
plt.tight_layout()
plt.savefig(outdir / "6_Heatmap.png", dpi=600)
plt.show()

print("\nALL 6 FIGURES SUCCESSFULLY SAVED WITH TINY, BEAUTIFUL POINTS")
print("→ Heatmap, bar plot, histogram, and all analysis 100% unchanged")
